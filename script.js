...
function loadContent() {
    let btnRemove = document.querySelectorAll('.cart-remove');          //delete item
    btnRemove.forEach((btn) => {
        btn.addEventListener('click', removeItem);
    });

    let qtyElements = document.querySelectorAll('.cart-quantity');
    qtyElements.forEach((input) => {                                  //if we add items, price will increase
        input.addEventListener('change', changeQty);
    });

    let cartBtns = document.querySelectorAll('.add-cart');
    cartBtns.forEach((btn) => {                                      //In cart item present ,still we can add
...
function addCart() {
    ...
    if(itemList.find((el)=>el.title==newProduct.title))          //can't add more than one time
    {
    ...
    let newProductElement=createCartProduct(title,price,imgSrc);    //display
...
function updateTotal()
{
    ...
    cartItems.forEach(product=>{                          //forloop
